...
2. **Detail the steps needed to perform the research:**
   ...
   - Lay out what @explore should look for
   - Specify what patterns @explore should look for
   - Be clear that explorers collect information for oracles
   - Typically run a single @oracle
...
3. **Spawn tasks for comprehensive research (follow this sequence):**
   ...
   **Phase 1 - Locate (Codebase & Thoughts):**
   ...
   - Spawn @explore agents in parallel for each topic group to find WHERE files and components live
   - Simultaneously spawn @explore agents in parallel to discover relevant documents
   - **WAIT** for all explore agents to complete before proceeding

   **Phase 2 - Find Patterns (Codebase only):**
   - Based on explorer results, identify patterns you need to find
   - Use @explore agents to find examples of similar implementations
   - Run multiple @explore agents in parallel if searching for different unique patterns
   - **WAIT** for all explore agents to complete before proceeding

   **Phase 3 - Analyze (Codebase & Thoughts):**
   - Using information from explorers, determine what needs deep analysis
   - Group analysis tasks by topic/component
   - Spawn @oracle agents in parallel for each topic group to understand HOW specific code works
   - Spawn @oracle agents in parallel to extract key insights from the most relevant documents found
   - **WAIT** for all oracle agents to complete before synthesizing

   **Important sequencing notes:**
   - Each phase builds on the previous one - explorers inform analysis
